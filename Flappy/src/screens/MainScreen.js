    ...
    var MainScreen = Ω.Screen.extend({
        ...
        tick: function () {
            ...
            if(valid) {
	            ...
	            if(this.gameplay>0 && this.gameplay<400 && this.gameplay%100 == 0 && toPrint) {
	            	console.log("HIGH SCORE SO FAR: "+window.game.best);
	            	this.bird.handleop1(this.Scores, this.gameplay);
	            	var qStr="";
	            	for (var i = 0; i < (this.vertical_dist_range[1] - this.vertical_dist_range[0])/this.blockSize; i++) {
	    				for (var j = 0; j < (this.horizontal_dist_range[1] - this.horizontal_dist_range[0])/this.blockSize; j++) {
	    					qStr+=i+"$$"+j+"$$fly$$"+this.Q[i][j]["fly"]+"$$stay$$"+this.Q[i][j]["stay"]+"\n";
	    				}
	    			}
	            	console.log(qStr);
	            	
	            } else if(this.gameplay>400 && this.gameplay%10 == 0 && toPrint) {
	            	console.log("HIGH SCORE SO FAR: "+window.game.best);
	            	this.bird.handleop1(this.Scores, this.gameplay);
	            	var qStr="";
	            	for (var i = 0; i < (this.vertical_dist_range[1] - this.vertical_dist_range[0])/this.blockSize; i++) {
